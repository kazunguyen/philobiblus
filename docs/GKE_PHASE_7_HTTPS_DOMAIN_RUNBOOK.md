...
`gke-app` lấy `gateway_host`, `gateway_https_enabled`, `gateway_certificate_map_name` bằng typed variables/remote state platform. Sửa `scripts/k8s-terraform/30-app-apply.sh` để render local ignored `terraform.tfvars`, fail fast khi HTTPS enabled thiếu host/map, và update example với placeholders. Certificate map name không phải PEM, secret hay full self-link.
...
2. **Cách chạy và quản lý**:
   - Áp dụng proxy qua script: `bash scripts/k8s-terraform/35-proxy-apply.sh`.
   - Output cung cấp:
   ...
   - Thu hồi proxy khi đã cấu hình xong custom domain: `bash scripts/k8s-terraform/36-proxy-destroy.sh`.
