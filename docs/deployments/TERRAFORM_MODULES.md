...
Mỗi module có `terraform.tfvars.example` chứa placeholder. Tạo file `terraform.tfvars` thật ở local hoặc để scripts tạo trong `scripts/gcp-gke/.local`; không stage các file này. Không commit:
...
Nạp secret value sau `foundation` bằng `scripts/gcp-shared/terraform-secrets-apply.sh`; Terraform chỉ quản lý Secret Manager resource và IAM, không quản lý secret value.

## 7. Bản đồ scripts

| Mục tiêu | Script chính |
| --- | --- |
| Bootstrap + foundation | `scripts/gcp-shared/terraform-apply.sh` |
| Destroy foundation/bootstrap có guard | `scripts/gcp-shared/terraform-delete.sh` |
| Cloud Run runtime | `scripts/gcp-cloud-run/terraform-runtime-apply.sh`, `scripts/gcp-cloud-run/terraform-runtime-delete.sh` |
| GKE platform và application | `scripts/gcp-gke/deploy.sh`, hoặc các bước `15-platform-plan.sh` → `20-platform-apply.sh` → `25-kubeconfig.sh` → `27-core-secrets.sh` → `30-app-apply.sh` |
| Cloud Run HTTPS proxy cho GKE | `scripts/gcp-gke/35-proxy-apply.sh`, `36-proxy-destroy.sh` |
| Liệt kê state Cloud Run cũ | `scripts/gcp-cloud-run/terraform-managed-resources.sh` (hiện chỉ đọc bootstrap/foundation/runtime; không phải inventory đầy đủ cho GKE modules) |

Xem [GKE_DEPLOYMENT_PLAN.md](gcp-gke/GKE_DEPLOYMENT_PLAN.md) cho lộ trình GKE và [GKE_PHASE_7_HTTPS_DOMAIN_RUNBOOK.md](gcp-gke/GKE_PHASE_7_HTTPS_DOMAIN_RUNBOOK.md) cho custom domain/HTTPS chính thức.
