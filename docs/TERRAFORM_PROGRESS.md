...
Use `bash scripts/terraform-secrets-apply.sh` only for a fresh environment. It creates the Cloud SQL application user and new secret versions without exposing values in Terraform state or `terraform.tfvars`.
