...
case "$MODE" in
  ...
  *)
    echo "Usage: bash scripts/gcp-shared/terraform-secrets-apply.sh [--core-only|--imgbb-only]" >&2
    exit 2
    ;;
esac
...
[[ -n "$PROJECT_ID" ]] || {
  echo "Set PROJECT_ID; see scripts/gcp-cloud-run/deploy-gcp.env.example." >&2
  exit 1
}
...
versions, rerun with: ROTATE_DB_PASSWORD=true bash scripts/gcp-shared/terraform-secrets-apply.sh
EOF
    exit 1
  fi
