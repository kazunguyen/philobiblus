...
[[ -f "$RUNTIME_DIR/terraform.tfvars" ]] || {
  echo "Missing $RUNTIME_DIR/terraform.tfvars. Run scripts/gcp-cloud-run/terraform-configure.sh first." >&2
  exit 1
}

if ! grep -Eq '^[[:space:]]*backend_image[[:space:]]*=' "$RUNTIME_DIR/terraform.tfvars" && \
   [[ ! -f "$RUNTIME_DIR/images.auto.tfvars" ]]; then
  echo "Missing runtime image references. Run scripts/gcp-shared/dockerhub-build-push.sh first." >&2
  exit 1
fi
